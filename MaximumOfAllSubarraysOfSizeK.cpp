  ...
  public:
    ...
    vector <int> max_of_subarrays(int *arr, int n, int k)
    {
        ...
        for(int i=0;i<n;i++)
        {
            ...
            for(int j=max(i-(k-1),lr[i].first);j<=i&&j+(k-1)<=lr[i].second;j++)
                ans[j]=arr[i];
        }
        return ans;
    }
